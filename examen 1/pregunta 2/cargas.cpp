...
double fxy (double x, double y)
{
 ...
 f=3/sqrt(x*x+y*y)+2/sqrt((x-1)*(x-1)+(y-1)*(y-1))+2/sqrt((x-1)*(x-1)+y*y)+2/sqrt((y-1)*(y-1)+x*x);
 return f;
}
...
int main()
{
 ...
 del=1e-6;
 h=1e-6;
 a=1; //le cambiamos de 0.1 a 0.001 para una mejor precisión
 ...
 do { //cambiamos el while por el do while para que se vayan guardando los valores
  ...
  file<<xo<<" "<<yo<<" "<<fxy(xo,yo)<<endl; //le agrego el cero puesto que estamos graficando en 3D y teniamos solamente dos columnas
  file<<endl;
  file<<endl;
