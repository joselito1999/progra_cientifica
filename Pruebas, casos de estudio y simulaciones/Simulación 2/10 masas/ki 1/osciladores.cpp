...
int main ()
{
 ...
 ofstream outfile3 ("vecprop.dat"); //Para sacar vectores propios y ponerlos en el esquema
 //ofstream outfile4 ("frecuencias_10_1.txt");
 m=20000;
 dt=0.01;
 read(fx);
 print(n,mat);
 ...
 //Sacar los vectores propios para graficar el esquema
  
 for(int i=n-1; i>-1; i--){
  for(int j=0; j<n; j++){
   outfile3<<2*j<<" "<<2*((n-1-i)+1)<<" "<<2*v[j][i]<<endl; //Le multiplico por 2 para una mejor visualización
  }
 }
