...
void inicio (ostream& fileout)//Define la forma inicial de la cuerda, es decir u(x,0)
{
 int m;
  m=ceil(xo/dx);//Se define m tal que divida x<xo y x>xo. Para inicializar el 
 //segmento 1
 for ( int i = 0; i < m; i++ ) {
  u[i]=h*dx*i/xo;
  fileout<<i*dx<<" "<<u[i]<<endl; //Almaceno la forma en un archivo
 }
 //segmento 2
...
double fbn (int n, double p)//Calculo bn. Por las condiciones iniciales solo unos es diferente de cero
{
 ...
 f=0;
 for ( int i = 0; i <= nn/2-1; i++ ) {//Se hace por la regla de Simpson
