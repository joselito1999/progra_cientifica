...
{
 double a,x,f,re,im,xj[2][2],x1[2][4];//Se aumenta una nueva matriz x1. El primer índice es real o complejo, los otros son los x e y. Con todos esto recién calculo los elementos de la matriz xj
 int aux;
 ...
 for ( int j = 0; j < N/20; j++ )
 {
  ...
  for ( int k = 0; k < N/4; k++ )//Solo voy hasta N/4, pues son 2 descomposiciones
  {
   a=2*pi*j*(4*k)/N;
   for ( int i = 0; i < 4; i++ ) {
    filein>>x>>f;  
  ...
  a=2*pi*j/N;
  for ( int i = 0; i < 2; i++ ) {//Calculo los xj yj, usando las sumas anteriores.
