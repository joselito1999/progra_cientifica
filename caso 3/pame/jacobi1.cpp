...
int main(int argc, char *argv[])
{
 ...
 c=1;
 do { //calculo iterativo
  for ( int i = 0; i < n; i++ )
   {
    for ( int j =i+1 ; j < n; j++ )
     {
      if (mat[i][j]!=0) {
       aa=(mat[j][j]-mat[i][i])/mat[i][j]; //alpha
       t=(-aa+sqrt(aa*aa+4))/2; //t
       //construcción de la matriz Pij
       ...
       print(n,r); 
       cout<<endl;
       product(n,r,r,p); //calculo de vp
       product(n,aux,mat,p); //producto matriz*p
       productt(n,mat,p,aux); //transpuesta p y la matriz anterior aux
       
       //le hacemos diagonal de nuevo a P
       p[i][i]=p[j][j]=1;
       p[i][j]=p[j][i]=0;
      } 
     }
...
//	  cout<<mat[i][i]<<" "<<sqrt(mat[i][i])<<"w0"<<endl;
 // }
  ...
  for ( int i = 0; i < n; i++ ) //cálculo del parámetro n.
   ...
   cout<<endl;
  c++;
  } while (sqrt(tol)>0.0001 and c<300);
